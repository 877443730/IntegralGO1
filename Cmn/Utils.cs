...
namespace NWJ.Cmn
{
    public class Utils
    {
        ...
        /// <returns>加密后的32位MD5码</returns>
        public static string MD5StringEncoding32(string ConvertString)
        {
            string pwd = "";
            MD5 md5 = System.Security.Cryptography.MD5.Create();//实例化一个md5对像
            // 加密后是一个字节类型的数组，这里要注意编码UTF8/Unicode等的选择
            byte[] convertByte = md5.ComputeHash(Encoding.UTF8.GetBytes(ConvertString));
            // 通过使用循环，将字节类型的数组转换为字符串，此字符串是常规字符格式化所得
            for (int i = 0; i < convertByte.Length; i++)
            {
                // 将得到的字符串使用十六进制类型格式。格式后的字符是小写的字母，
                //如果使用大写（X）则格式后的字符是大写字符
                pwd = pwd + convertByte[i].ToString("X");
            }
            return pwd;
        }
        #endregion
        ...
        /// 检测是否符合手机号码格式
        /// </summary>
        /// <param name="strEmail">要判断的手机号字符串</param>
        /// <returns>判断结果</returns>
        public static bool IsValidMobile(string strMobile)
        {
            return Regex.IsMatch(strMobile, @"^1[34578]\d{9}$");
        }
        /// <summary>
        /// 检测是否是正确的Url
        /// </summary>
        ...
        /// <param name="defValue">缺省值</param>
        /// <returns>转换后的int类型结果</returns>
        public static string StrToDateTime(string str, string defValue)
        {
            if (!string.IsNullOrEmpty(str))
            {
                try
        ...
        /// <param name="obj">要转换的对象</param>
        /// <returns>转换后的string类型结果</returns>
        public static string ObjectToStrEmpty(object obj)
        {
        ...
        public static string DropHTML(string Htmlstring)
        {
            ...
            Htmlstring = Regex.Replace(Htmlstring, @"&(rdquo|ldquo);", "\"", RegexOptions.IgnoreCase);
            Htmlstring.Replace("<", "");
            Htmlstring.Replace(">", "");
            Htmlstring.Replace("\r\n", "");
        ...
        public static void DeleteUpFile(string _filepath,string file= "/thumb_")
        {
            if (string.IsNullOrEmpty(_filepath))
            {
                return;
            ...
            {
                string thumbnailpath = _filepath.Substring(0, _filepath.LastIndexOf("/")) + file + _filepath.Substring(_filepath.LastIndexOf("/"));
        ...
        /// <param name="centSize">中间页码数量</param>
        /// <returns></returns>
        public static string NWJOutPageList(int pageSize, int pageIndex, int totalCount, string linkUrl, int centSize)
        {
            //计算页数
            if (totalCount < 1 || pageSize < 1)
            {
            ...
            StringBuilder pageStr = new StringBuilder();
            string pageId = "__id__";
            string firstBtn = "<li><a href=\"" + ReplaceStr(linkUrl, pageId, (pageIndex - 1).ToString()) + "\" aria-label=\"Previous\"><span aria-hidden=\"true\">&laquo;</span></a></li>";
            //string firstBtn = "<a href=\"" + ReplaceStr(linkUrl, pageId, (pageIndex - 1).ToString()) + "\">«上一页</a>";
            //string lastBtn = "<a href=\"" + ReplaceStr(linkUrl, pageId, (pageIndex + 1).ToString()) + "\">下一页»</a>";
            string lastBtn = "<li><a href=\"" + ReplaceStr(linkUrl, pageId, (pageIndex + 1).ToString()) + "\" aria-label=\"Next\"><span aria-hidden=\"true\">&raquo;</span></a></li>";
            ...
            if (pageIndex <= 1)
            {
                firstBtn = "<li><span aria-hidden=\"true\" >&laquo;</span></li>";
            }
            if (pageIndex >= pageCount)
            {
                lastBtn = "<li><span class=\"disabled\">&raquo;</span></li>";
            }
            if (pageIndex == 1)
            {
            ...
            if (lastNum >= pageCount)
                lastNum = pageCount - 1;
            pageStr.Append("<li><span>共" + totalCount + "记录</span></li>");
            pageStr.Append(firstBtn + firstStr);
            if (pageIndex >= centSize)
            {
                pageStr.Append("<li><span aria-hidden=\"true\">...<span></li>\n");
            }
            for (int i = firstNum; i <= lastNum; i++)
            {
                if (i == pageIndex)
                {
                    pageStr.Append("<li class=\"active\"><a href=\"javascript:;\">" + i + "</a></li>");
                }
                else
                {
        ...
        /// <param name="centSize">中间页码数量</param>
        /// <returns></returns>
        public static string NWJRoteOutPageList(int pageSize, int pageIndex, int totalCount, string linkUrl, int centSize)
        {
            //计算页数
            if (totalCount < 1 || pageSize < 1)
            {
